...
<body>
	...
	<table>
		<tr>
			<th id="box">
			  	<div>
			  		<h4 id="header">REMOVE PRODUCT STOCK</h4>
			  		<form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>"  name="add_stock" autocomplete="off">
						...
						<button class="login_button" type="submit">Remove Stock</button>
			  		</form>
						...
						if ($new_quantity >= 0) { 	
							...
							if (mysqli_query($conn, $sql)) {
								...
								<script type="text/javascript">
									...
									window.location.replace('remove_stock.php');
								</script>
								<?php
							} else {
								echo "query failed";
							...
							?>
							<script type="text/javascript">
								alert("Please Give Proper Quantity Which Should be greater than Available Quantity!!");
							</script>
							<?php
						}
					}
