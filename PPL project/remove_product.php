...
<body>
	...
	<table>
		<tr>
			<th id="box">
			  	<div>
			  		<h4 id="header">REMOVE PRODUCT</h4>
			  		<form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>"  name="add_stock" autocomplete="off">
						...
						<button class="login_button" type="submit">Remove Product</button>
			  		</form>
	...
	<?php 
		if ($done == 1) {	
			...
			if ($conn = mysqli_connect($db_servername, $db_username, $db_password, $db1_name)) {
				$sql = "SELECT product_id FROM products_stock_table WHERE product_id = '".$product_id."'";
				if ($result = mysqli_query($conn, $sql)) {
					...
					if ($row == 0) {
						...
						$sql = "DELETE FROM products_stock_table
								WHERE product_id = '".$product_id."'";
						if (mysqli_query($conn, $sql)) {
							?>
							<script type="text/javascript">
								alert("Product Removed Successfully!!");
								window.location.replace('remove_product.php');
							</script>
							<?php
						} else {
							echo "query failed";
